...
pub fn declaration_func(input: &Input, declaration : &ConstSizedFunctionDeclaration, alignment_module : &syn::Ident) -> proc_macro2::TokenStream {
    ...
    if *send{
        out.extend(quote! {
            unsafe impl #generics core::marker::Send for #struct_name #generics{}
        });
    }
    if *sync{
        out.extend(quote! {
            unsafe impl #generics core::marker::Sync for #struct_name #generics{}
        });
    }
...
pub fn drop(declaration : &ConstSizedFunctionDeclaration, pollable_ident: &syn::Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl #lifetimes core::ops::Drop for #name #lifetimes{
            fn drop(&mut self) {
                unsafe{
